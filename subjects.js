...
const maybeLogInForm = async (page) => {
    const formWithMsg = await page.$('form[action="https://campus.exactas.uba.ar/login/index.php"] div.felement.fstatic');
    ...
    const formNoMsg = await page.$('form[action="https://campus.exactas.uba.ar/enrol/index.php"] legend.ftoggler');
...
const maybeLogInNotice = async (page) => {
    ...
    const areGuestNotAllowedMsg = guestsMaybeNotallowed ? await page.evaluate(el => el.textContent, guestsMaybeNotallowed) : null;

    return areGuestNotAllowedMsg && (areGuestNotAllowedMsg.trim().toLowerCase() === NOTALLOWEDMSG || areGuestNotAllowedMsg.trim().toLowerCase() === NOTALLOWEDMSGEN)
}
...
(async () => {
    ...
    try {
        for (year of subjectsByYear) {
            for (periods of year.periods) {
                for(periodContent of periods.content) {
                    ...
                    if (await page.$('#notice > div > div:nth-child(1) > form > div > input[type=submit]:nth-child(1)') !== null) {
                        await page.click('#notice > div > div:nth-child(1) > form > div > input[type=submit]:nth-child(1)');
                        await page.waitForSelector('div[role="main"]');
                    }
                    ...
                    const tabs = await page.$$eval(
                        'div.course-content ul.nav.nav-tabs > li > a',
                        xs => xs.map(a => ({ tab: a.title || 'Inicio', href: a.href || null })),
                    );
