...
const prepareDataToDownload = (grossData) => {
    return grossData.flatMap(byYear => {
        const publicSubjects = byYear.periods.flatMap(period => period.content.filter(ss => !ss.needAuth));
...
(async () => {
    ...
    try {
        ...
        for (yearData of processedData) {
            for (subject of yearData.subjects) {
                for (subjectTab of subject.tabs) {
                    ...
                    if (foldersToDownload.length) {
                        for (el of foldersToDownload) {
                            ...
                            await downloadWithElementHandle(tab, submitElement, `${BASE_DOWNLOAD_PATH}/${yearData.year}/${subject.subject}`)
